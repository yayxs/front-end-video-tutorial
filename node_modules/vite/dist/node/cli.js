...
var build = require('./chunks/dep-cb562f8f.js');
...
async function preview(config, serverOptions) {
    ...
    const httpServer = await build.resolveHttpServer(config.server, app, serverOptions.https === false ? undefined : await build.resolveHttpsConfig(config));
    // cors
    ...
    httpServer.listen(port, hostname.host, () => {
        ...
        if (options.open) {
            ...
            build.openBrowser(`${protocol}://${hostname.name}:${port}${path}`, true, logger);
        }
    });
}
...
cli
    ...
    // is ok here
    const { createServer } = await Promise.resolve().then(function () { return require('./chunks/dep-cb562f8f.js'); }).then(function (n) { return n.index$1; });
    try {
...
cli
    ...
    .action(async (root, options) => {
    const { build: build$1 } = await Promise.resolve().then(function () { return require('./chunks/dep-cb562f8f.js'); }).then(function (n) { return n.build$1; });
...
cli
    ...
    .option('--force', `[boolean] force the optimizer to ignore the cache and re-bundle`)
    .action(async (root, options) => {
    const { optimizeDeps } = await Promise.resolve().then(function () { return require('./chunks/dep-cb562f8f.js'); }).then(function (n) { return n.index; });
    try {
