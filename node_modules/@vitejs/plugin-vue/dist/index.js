...
var require_hash_sum = __commonJS({
  "../../node_modules/hash-sum/hash-sum.js"(exports, module2) {
    "use strict";
...
function compile(code, descriptor, options, pluginContext, ssr) {
  ...
  const result = (0, import_compiler_sfc2.compileTemplate)(__spreadProps(__spreadValues({}, resolveTemplateCompilerOptions(descriptor, options, ssr)), {
    source: code
  }));
...
function resolveTemplateCompilerOptions(descriptor, options, ssr) {
  ...
  return __spreadProps(__spreadValues({}, options.template), {
    ...
    preprocessOptions,
    compilerOptions: __spreadProps(__spreadValues({}, (_c = options.template) == null ? void 0 : _c.compilerOptions), {
...
function resolveScript(descriptor, options, ssr) {
  ...
  let resolved = null;
  resolved = (0, import_compiler_sfc3.compileScript)(descriptor, __spreadProps(__spreadValues({}, options.script), {
    id: descriptor.id,
    isProd: options.isProduction,
...
async function transformMain(code, filename, options, pluginContext, ssr) {
  ...
  if (ssr) {
    output.push(`import { useSSRContext as __vite_useSSRContext } from 'vue'`, `const _sfc_setup = _sfc_main.setup`, `_sfc_main.setup = (props, ctx) => {`, `  const ssrContext = __vite_useSSRContext()`, `  ;(ssrContext.modules || (ssrContext.modules = new Set())).add(${JSON.stringify(filename)})`, `  return _sfc_setup ? _sfc_setup(props, ctx) : undefined`, `}`);
  }
  let resolvedMap = map;
  if (map && templateMap) {
...
async function genScriptCode(descriptor, options, pluginContext, ssr) {
  ...
  if (script) {
    if ((!script.lang || script.lang === "ts" && options.devServer) && !script.src) {
      ...
      scriptCode = (0, import_compiler_sfc4.rewriteDefault)(scriptCode, `_sfc_main`);
    } else {
      if (script.src) {
...
async function transformStyle(code, descriptor, index, options, pluginContext) {
  ...
  const result = await (0, import_compiler_sfc5.compileStyleAsync)(__spreadProps(__spreadValues({}, options.style), {
