...
function createMarkdownToVueRenderFn(root, options = {}, pages) {
    const md = markdown_1.createMarkdownRenderer(root, options);
    pages = pages.map((p) => slash_1.slash(p.replace(/\.md$/, '')));
    return (src, file, publicDir) => {
        const relativePath = slash_1.slash(path_1.default.relative(root, file));
        ...
        if (data.links) {
            ...
            for (let url of data.links) {
                ...
                const resolved = slash_1.slash(url.startsWith('/')
                    ? url.slice(1)
                    : path_1.default.relative(root, path_1.default.resolve(dir, url)));
